...
medage <- get_acs(geography = "tract", state = "MD", county="Montgomery",table = "B01002") %>% 
  filter(variable == "B01002_001") %>% 
  select(-variable,-moe,-NAME) %>% 
  rename(medianage = estimate)

raceeth <- get_acs(geography = "tract", state = "MD", county="Montgomery", table = "B03002") %>% 
...
citizenship <- get_acs(geography = "tract", state = "MD", county="Montgomery",table="B05001") %>% 
  pivot_wider(id_cols=GEOID,names_from = variable, values_from = estimate) %>% 
...
moving <- get_acs(geography = "tract", state = "MD", county="Montgomery",table = "B07003") %>%  
  pivot_wider(id_cols = GEOID, names_from=variable, values_from=estimate) %>%
...
# HHtype
hhtype <- get_acs(geography = "tract", state = "MD", county="Montgomery",table="B11001") %>% 
  pivot_wider(id_cols = GEOID, names_from=variable, values_from=estimate) %>%
